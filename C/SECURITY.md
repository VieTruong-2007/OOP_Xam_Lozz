...
| File | Role | Risk |
|------|------|------|
| `C.cpp` | Game source | Low — local GUI only |
| `C.exe` | Compiled game | Rebuild from `C.cpp` to trust |
| `build.bat` | Compile / optional winget install | Low — fixed commands |
| `watch_build.ps1` | Auto-build watcher | Low — only calls `build.bat` |
| `*.bat` | Wrappers | Low |
| `theme_setup.txt` | Colors / background filename | Low — validated in code |
| `background_cache.jpg` | User image cache | Only image files in game folder |
...
1. Compare `C.cpp` with the repo you trust.
2. Delete `C.exe`, run `build.bat` to rebuild.
