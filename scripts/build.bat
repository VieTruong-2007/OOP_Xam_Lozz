...
if not exist "src\C.cpp" (
    echo ERROR: src\C.cpp not found in %CD%
    exit /b 1
)
...
if "!USE_MSVC!"=="1" (
    cl.exe /EHsc /nologo /utf-8 /DUNICODE /D_UNICODE "src\C.cpp" /Fe"dist\C.exe" /Fo"dist\C.obj" /link /subsystem:windows user32.lib gdi32.lib kernel32.lib gdiplus.lib comdlg32.lib
    set "RC=!errorlevel!"
    goto :compile_done
)

"!COMPILER_CMD!" -std=c++17 -O2 -DUNICODE -D_UNICODE -mwindows -municode "src\C.cpp" -o "dist\C.exe" -lgdi32 -luser32 -lkernel32 -lgdiplus -lcomdlg32
set "RC=!errorlevel!"

:compile_done
if !RC! equ 0 (
    echo Build successful: %CD%\dist\C.exe
) else (
    echo Build FAILED.
)
exit /b !RC!
...
:get_cpp_stamp
for %%F in ("src\C.cpp") do set "%~1=%%~tF"
exit /b 0

:launch_exe
if not exist "dist\C.exe" (
    echo Cannot run: dist\C.exe does not exist.
    exit /b 1
)
